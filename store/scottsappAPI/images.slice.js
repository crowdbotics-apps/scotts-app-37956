...
export const modules_camera_photos_user_read = createAsyncThunk(
  "images/modules_camera_photos_user_read",
  async payload => {
    const response = await apiService.modules_camera_photos_user_read(payload)
    return response.data
  }
)
...
const imagesSlice = createSlice({
  ...
  extraReducers: {
    ...
    [modules_camera_photos_user_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [modules_camera_photos_user_read.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [
    ...
    [modules_camera_photos_user_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
