...
const termAndConditionsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [modules_terms_and_conditions_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [modules_terms_and_conditions_read.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [
    ...
    [modules_terms_and_conditions_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
    ...
    [modules_terms_and_conditions_delete.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [modules_terms_and_conditions_delete.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
    ...
    [modules_terms_and_conditions_delete.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error
        state.api.loading = "idle"
      }
